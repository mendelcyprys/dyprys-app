...
export function App() {
  ...
  React.useEffect(() => {
    ...
    const fallback = rows.find((row) => row.default && row.exists) ?? rows.find((row) => row.exists);
    if (fallback && fallback.name !== library) select(fallback.name);
  }, [libraries.data, library, select]);

  if (libraries.isLoading) {
    return <Shell><div className="p-10 text-sm text-muted-foreground">Loading…</div></Shell>;
  }
  if (libraries.error instanceof DyprysError && libraries.error.kind === "unreachable") {
    return <Shell><ServerUnreachable /></Shell>;
  }
  if (!libraries.data) {
    return <Shell><ServerUnreachable /></Shell>;
  }
  if (!libraries.data.registry_readable) {
    return <Shell><RegistryUnreadable path={libraries.data.registry_path} /></Shell>;
  }
  if (libraries.data.libraries.length === 0) {
    return <Shell><NothingRegistered /></Shell>;
  }
